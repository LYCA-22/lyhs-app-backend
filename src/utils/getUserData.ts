...
export async function getUserById(userId: string, ctx: AppContext): Promise<userData | Response> {
	const query =
		'SELECT id, name, email, type, level, class, grade, role, number, created_person, oauth, stu_id FROM accountData WHERE id = ?';
...
export async function getUserByEmail(email: string, ctx: AppContext): Promise<userData | Response> {
	const query =
		'SELECT id, name, email, type, level, class, grade, role, number, created_person, oauth, stu_id FROM accountData WHERE email = ?';
