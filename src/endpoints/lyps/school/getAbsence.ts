...
export class getAbsence extends OpenAPIRoute {
	async handle(ctx: AppContext) {
		...
		try {
			const res = await fetch(
				`
				https://highschool.kh.edu.tw/B0209S_Absence_select.action?_search=false&nd=1752580739257&rows=300&page=1&sidx=&sord=asc&session_key=${sessionKey}`,
				{
					method: 'POST',
					headers: {
						'User-Agent':
