	...
	async handle(ctx: AppContext) {
		...
		try {
			const res = await fetch(
				`https://highschool.kh.edu.tw/A0410S_StdSyear_select.action?queryKind=a0410s&_search=false&nd=1748777267358&rows=-1&page=1&sidx=syear&sord=asc&session_key=${sessionKey}`,
				{
					method: 'POST',
					headers: {
						'User-Agent':
